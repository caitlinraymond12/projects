...
def findShortest(initial):
  global puzzle
  closed = set()
  closed.add(hashdict(initial.states))
  keys = puzzle.keys()
  q = queue()
  q.add(initial)
  while not q.empty():
    ...
    if currState == None:
      continue
    newState = switchSides(currState, "farmer")
    if newState:
      if success(newState.states):
        return newState
      hv = hashdict(newState.states)
      if safe(newState.states) and hv not in closed:
        q.add(newState)
        closed.add(hv)
    for k in keys:
